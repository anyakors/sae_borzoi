...
mkdir -p "${OUTPUT_DIR}/${DATE_STAMP}/meme_out"
mkdir -p "${OUTPUT_DIR}/${DATE_STAMP}/tomtom_out"
...
for fa_file in "${INPUT_DIR}"/node_seqs_sorted_500/*.fa; do
    if [ -f "$fa_file" ]; then
        ...
        ${MEME_INSTALL_DIR}/src/meme "$fa_file" \
            -o "${OUTPUT_DIR}/${DATE_STAMP}/meme_out/${filename}" \
            -dna \
            -nmotifs 2 \
            -oc "${OUTPUT_DIR}/${DATE_STAMP}/meme_out/${filename}"

        # Check if MEME was successful
        if [ $? -eq 0 ]; then
            echo "MEME analysis completed for $filename"

            python ./meme-parser.py "${OUTPUT_DIR}/${DATE_STAMP}/meme_out/${filename}/meme.txt" -o "${OUTPUT_DIR}/${DATE_STAMP}/meme_out/${filename}/motifs.tsv"
            
            # Run TomTom on MEME output
            echo "Running TomTom on MEME results for $filename..."
            ${MEME_INSTALL_DIR}/src/tomtom \
                "${OUTPUT_DIR}/${DATE_STAMP}/meme_out/${filename}/meme.txt" \
                "${MEME_INSTALL_DIR}/motif_databases/CIS-BP_2.00/Homo_sapiens.meme" \
                -o "${OUTPUT_DIR}/${DATE_STAMP}/tomtom_out/${filename}"
...
Processed Files:
$(ls -1 "${INPUT_DIR}"/node_seqs_sorted_500/*.fa)
