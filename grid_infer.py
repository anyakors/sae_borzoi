...
for exp_factor in params['expansion_factors']:
    for topk in params['topk_pct']:
        for lr in params['learning_rate']:
            ...
            slurm_string += f"#SBATCH -o /home/anya/code/sae_borzoi/{model_save_path}/job0_infer.out \n"
            slurm_string += f"#SBATCH -e /home/anya/code/sae_borzoi/{model_save_path}/job0_infer.err \n"
            slurm_string += '#SBATCH --mem 28000 \n#SBATCH --time 1-0:0:0 \n'
            slurm_string += '#SBATCH --gres=gpu:1 \n'  # Request 1 GPU
            slurm_string += f'. /home/anya/anaconda3/etc/profile.d/conda.sh; conda activate basenji-torch2; echo $HOSTNAME; python infer_one_instance.py --topk {topk} --exp_factor {exp_factor} --lr {lr} --top_acts 16'
