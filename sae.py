...
class SparseAutoencoder(nn.Module):
    ...
    def get_sparse_activations(self, h):
        ...
        if self.sparsity_method == 'topk':
            ...
            threshold = h.abs().mean(dim=1, keepdim=True) + h.abs().std(dim=1, keepdim=True)
            return h * (h.abs() >= threshold)
    ...
    def get_metrics(self, h_sparse):
        ...
        neuron_diversity = active_neurons.nonzero().size(0) / h_sparse.size(1)
        
        # Dead neuron count (never activated in batch)
        dead_neurons = (active_neurons == 0).sum()
        
        # Activation statistics
        mean_activation = h_sparse.abs().mean()
        std_activation = h_sparse.abs().std()
        
        return {
            'sparsity_ratio': sparsity_ratio.mean().item(),
            'neuron_diversity': neuron_diversity.item(),
            'dead_neurons': dead_neurons.item(),
